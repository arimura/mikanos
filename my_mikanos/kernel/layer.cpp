...
Vector2D<int> Layer::GetPosotion() const {
  return pos_;
}
...
void LayerManager::Draw(unsigned int id) const {
  ...
  for (auto layer : layer_stack_) {
    if (layer->ID() == id) {
      ...
      window_area.pos = layer->GetPosotion();
      draw = true;
    }
    if (draw) {
...
void LayerManager::Move(unsigned int id, Vector2D<int> new_pos) {
  ...
  const auto old_pos = layer->GetPosotion();
  layer->Move(new_pos);
...
Layer* LayerManager::FindLayerByPosition(Vector2D<int> pos, unsigned int exclude_id) const {
  auto pred = [pos, exclude_id](Layer* layer) {
    ...
    const auto win_pos = layer->GetPosotion();
...
void ActiveLayer::Activate(unsigned int layer_id) {
  if (activate_layer_ == layer_id) {
    return;
  }

  if (activate_layer_ > 0) {
    Layer* layer = manager_.FindLayer(activate_layer_);
    layer->GetWindow()->Deactivate();
    manager_.Draw(activate_layer_);
  }

  activate_layer_ = layer_id;
  if (activate_layer_ > 0) {
    Layer* layer = manager_.FindLayer(activate_layer_);
    layer->GetWindow()->Activate();
    manager_.UpDown(activate_layer_, manager_.GetHeight(mouse_layer_) - 1);
    manager_.Draw(activate_layer_);
  }
}
