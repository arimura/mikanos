...
namespace {
  ...
  Error ConfigureMSIRegister(const Device& dev, uint8_t cap_addr,
    ...
    if (msi_cap.header.bits.multiple_msg_capable <= num_vector_exponent) {
      msi_cap.header.bits.multiple_msg_ebable = msi_cap.header.bits.multiple_msg_capable;
    } else {
      msi_cap.header.bits.multiple_msg_ebable = num_vector_exponent;
    }
...
namespace pci {
  ...
  Error ConfigureMSI(const Device& dev, uint32_t msg_addr, uint32_t msg_data, unsigned int num_vector_exponent) {
    ...
    if (msi_cap_addr) {
      ...
      return ConfigureMSIRegister(dev, msix_cap_addr, msg_addr, msg_data, num_vector_exponent);
    }
