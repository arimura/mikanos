...
Task& Task::InitContext(TaskFunc* f, int64_t data) {
  const size_t stack_size = kDefalultStackBytes / sizeof(stack_[0]);
...
void TaskManager::SwitchTask() {
  size_t next_task_index = currenct_task_index_ + 1;
  if (next_task_index >= tasks_.size()) {
    next_task_index = 0;
  }

  Task& currenct_task = *tasks_[currenct_task_index_];
  Task& next_task = *tasks_[next_task_index];
  currenct_task_index_ = next_task_index;

  SwitchContext(&next_task.Context(), &currenct_task.Context());
}
